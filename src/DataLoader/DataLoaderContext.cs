...
{
    /// <summary>
    /// Defines the boundary for <see cref="DataLoader"/> instances.
    /// </summary>
    public class DataLoaderContext
    {
        ...
        /// <summary>
        /// Retrieves a <see cref="DataLoader"/> with the given key, creating and storing one if none is found.
        /// </summary>
        public IDataLoader<TKey, TValue> GetDataLoader<TKey, TValue>(object name,
            Func<IEnumerable<TKey>, ILookup<TKey, TValue>> fetch)
        {
            return (IDataLoader<TKey, TValue>) _loaderCache.GetOrAdd(name, _ => new BatchLoader<TKey, TValue>(this, fetch));
        }
