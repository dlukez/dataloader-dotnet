...
{
    /// <summary>Provides methods for obtaining new and cached loader instances.</summary>
    public class DataLoaderFactory
    {
        ...
        /// <summary>Retrieves a cached loader for the given key, creating one if none is found.</summary>
        ...
        {
            return (IDataLoader<TKey, TReturn>)_cache.GetOrAdd(key, _ => new BatchDataLoader<TKey, TReturn>(fetchDelegate, _loadContext));
        }

        /// <summary>Retrieves a cached loader for the given key, creating one if none is found.</summary>
        public IDataLoader<TReturn> GetOrCreateLoader<TReturn>(object key, Func<Task<TReturn>> fetchDelegate)
        {
            return (IDataLoader<TReturn>)_cache.GetOrAdd(key, _ => new RootDataLoader<TReturn>(fetchDelegate, _loadContext));
        }
    }
}